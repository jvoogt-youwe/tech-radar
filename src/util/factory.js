...
const plotRadarGraph = function (_title, blips, currentRadarName, alternativeRadars) {
  document.title = 'Tech Radar'

  d3.selectAll('.loading').remove()
  const loaderGraph = document.getElementById('youwe-loader')
  if (loaderGraph) loaderGraph.style.display = 'none'
...
const Factory = function () {
  ...
  self.build = function () {
    ...
    const domainName = DomainName(window.location.search.substring(1)) || DomainName(paramId || '')
...
function plotLoading(content) {
  if (!featureToggles.UIRefresh2022) {
    ...
    const inputForm = document.querySelector('.input-sheet-form')
...
function plotErrorMessage(exception, fileType) {
  if (featureToggles.UIRefresh2022) {
    ...
    d3.selectAll('.loading').remove()
    const loaderEl = document.getElementById('youwe-loader')
...
function plotUnauthorizedErrorMessage() {
  ...
  if (!featureToggles.UIRefresh2022) {
    ...
    const loaderEl2 = document.getElementById('youwe-loader')
